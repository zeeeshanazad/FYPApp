    ...
    func getUserAndWallet(_ callback : ((_ success : Bool) -> Void)?){
        ...
        FIRDatabase.database().reference().child("UserDetails").child(Resource.sharedInstance().currentUserId!).observeSingleEvent(of: .value, with: { (snap) in
            if let data = snap.value as? NSDictionary {
                let newUser = CurrentUser(id: Resource.sharedInstance().currentUserId!, email: data["email"] as! String, userName: data["userName"] as! String, imageURL: data["image"] as! String, birthdate: data["birthDate"] as? Double, deviceIDs: nil, subscriptionType: .none, lastSubscription: data["lastSubscription"] as? Double)
